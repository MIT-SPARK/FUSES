...
Tangent STPCG(const Variable &X, const Tangent &grad,
  ...
  double alpha_k; // Scalar used to compute the full Newton step along direction p_k
  double beta_k;
  double kappa_k; // Value of inner product < p_k, H*p_k >

  double skplus1_M_2; // norm of proposed (full) step
  ...
  while (num_iterations < max_iterations) {
    ...
    if (metric(X, g_k, g_k, args...) <= target_grad_norm * target_grad_norm) {
    ...
    skplus1_M_2 = sk_M_2 + 2 * alpha_k * sk_M_pk + alpha_k * alpha_k * pk_M_2;
...
TNT(const Objective<Variable, Args...> &f,
  ...
  // Methods"
  for (unsigned int iteration = 0; iteration < params.max_iterations; iteration++) { // outer Trust Region Iterations
