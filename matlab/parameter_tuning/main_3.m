...
% Objective function: sum of ||Q(lambda1, lambda2) - Q_estimated||_F
% Date: 1/24/2018
...
variable Q(N+K, N+K) symmetric

obj = 0;
for j = 1:T
    obj = obj + norm(lambda1*Q1List{j}+lambda2*Q2List{j}+QcList{j} - Q, 'fro');
end
...
subject to
    for j = 1:T
        ...
        Q + sum == semidefinite(N+K);
%         lambda1*Q1List{j}+lambda2*Q2List{j}+QcList{j} + sum == semidefinite(N+K);
        (Q + sum)*ZList{j} == 0;
        cList{j}'*x(1:length(cList{j}), j) + trace(Q*ZList{j}) == 0;
    end
