...
retCode NetTRT::init(const std::string& device, const float& mem_percentage) {
  ...
  // Siyi added the line below (registerInput initially has two inputs)
  nvuffparser::UffInputOrder inputOrder = nvuffparser::UffInputOrder::kNCHW; 
  _parser->registerInput(_input_node.c_str(), inputDims, inputOrder);
 ...
 * @return     Exit code
 */
retCode NetTRT::infer(const cv::Mat& image, cv::Mat& mask, cv::Mat& prob, const bool verbose) {
  ...
  // for FUSES: normalize probability by using softmax
  for (unsigned int y = 0; y < h; ++y) {
    for (unsigned int x = 0; x < w; ++x) {
      std::vector<float> label_data(num_classes);
      for (unsigned int ch = 0; ch < num_classes; ++ch) {
        label_data[ch] = output_cMats[ch].at<float>(y, x);
      }
      // softmax
      auto beg = label_data.begin();
      auto end = label_data.end();
      auto const max_ele { *std::max_element(beg, end) };
      float exptot = 0;
      std::transform(beg, end, beg,
          [&](auto x){auto ex = std::exp(x - max_ele); exptot += ex; return ex;});
      // save normalized probability
      for (unsigned int ch = 0; ch < num_classes; ++ch) {
        output_cMats[ch].at<float>(y, x) = label_data[ch] / exptot;
      }
    }
  }
  ...
  cv::Mat _probmax(cv::Size(w, h), CV_32FC1);
  for (unsigned int y = 0; y < h; ++y) {
    int* row_argmax = argmax.ptr<int>(y);
    float* row_c = output.ptr<float>(y);
    float* row_probmax = _probmax.ptr<float>(y);
    for (unsigned int x = 0; x < w; ++x) {
      ...
      row_probmax[x] = max; // normalized max
      row_argmax[x] = max_c;
    }
  }
...
///*
// * @brief      Helper function for computing softmax of a vector
// *
// * @param[in]  beg   iterator pointing to the head of the input vector
// * @param[in]  end   iterator pointing to the tail of the input vector
// * @param[out] dest  iterator pointing to the head of the output vector
// */
//template <typename IterIn, typename IterOut = IterIn>
//void NetTRT::softmax (IterIn beg, IterIn end, IterOut dest = beg) {
//  auto const max_ele { *std::max_element(beg, end) };
//
//  float exptot = 0;
//  std::transform(beg, end, dest,
//      [&](float x){ auto ex = std::exp(x - max_ele); exptot += ex; return ex;});
//}
