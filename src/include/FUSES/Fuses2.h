...
/**
 * @file   Fuses2.h
 * @brief  Fast Understanding Via Semidefinite Segmentation - parametrization in -1/+!
...
/* ============================= FUSES =========================== */
class Fuses2 : public FusesBase {
 public:
  /** Fuses2 constructor: taking crf and options (optional)
   * Will generate a random initial guess */
  Fuses2(const CRFsegmentation::CRF& crf, const boost::optional<FUSESOpts&>
    options = boost::none);

  /** Fuses2 constructor: taking initial guess in matrix form (only for expert
   * users, will set r0 according to Y0) and options (optional) */
  Fuses2(const CRFsegmentation::CRF& crf, const gtsam::Matrix &Y0,
      const boost::optional<FUSESOpts&> options = boost::none);

  /** Fuses2 constructor: taking initial guess in vector form nodeLabels[i]
   * corresponds to class of node i and options (optional) */
  Fuses2(const CRFsegmentation::CRF& crf,
  ...
  /*
   * The formulation -1/+1 has a constant term in the cost that should be accounted for
   * to compare with the CRF cost
   */
  double offset;
  ...
  /* convert from matrix indices to linear indices: X -> vec(X) */
  inline size_t x_ind_K(const size_t node, const size_t label) const { return node*crf_.nrClasses+label; }

  /* convert from matrix indices to linear indices: X -> vec(X) */
  inline size_t x_ind_N(const size_t node, const size_t label) const { return label*crf_.nrNodes+node; }

  /** Generate random initial guess for Y */
  void Y_rand();

  /** Generate initial guess using unary factors in CRF + assign label randomly if no unary
   * factor is provided */
  void Y_from_unary();
  ...
  double evaluate_objective(const gtsam::Matrix &Y) const {
    return (Y.transpose() * (Q_.selfadjointView<Eigen::Upper>() * Y)).trace() + offset;
  }
