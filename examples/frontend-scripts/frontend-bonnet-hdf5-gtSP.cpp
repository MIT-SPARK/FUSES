...
/**
 * @file   frontend-bonnet-hdf5-gtSP.cpp
 * @brief  use ground truth labels for superpixels
...
int main(int argc, char **argv) {
  ...
  string resultFolder = "results-gtSP";
  ...
  for(size_t i=0; i<nrImages; ++i) {
    ...
    // ground truth
    string cLabelFileName = labelImageName;
    cLabelFileName.replace(cLabelFileName.end()-19, cLabelFileName.end(), "gtFine_color.png");
    cv::Mat gtImage = cv::imread(cLabelFileName, cv::IMREAD_COLOR);

    cv::Mat inputCopy, converted, mask, labels;
    gtImage.copyTo(inputCopy);
    ...
    // correct binary
      ...
      sfe.correctBinary(labelsGT); // 1 and -1
    }
