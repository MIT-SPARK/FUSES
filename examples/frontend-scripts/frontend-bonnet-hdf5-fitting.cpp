 ...
 * @brief  take segmentation param lambda1 and lambda2 as input
...
int main(int argc, char **argv) {
  ...
  string textFileName = "tempNames-fitting.txt";
  string resultFolder = "results-fitting";
  ...
  segParam.lambda1 = 0;
  segParam.lambda2 = 1; // store exp( -colorDiff * beta) and reparametrize later
  ...
  for(size_t i=0; i<nrImages; i++) {
    ...
    // eg.'results-fitting/aachen_000000_000019_leftImg8bit.png'
    frameName = resultFolder + "/" + frameName;

    string modelName = frameName;     // eg.'results-fitting/aachen_000000_000019.h5';
    modelName.replace(modelName.end()-16, modelName.end(), ".h5");

    string labelFileName = frameName; // eg.'results-fitting/aachen_000000_000019_labels.csv';
    labelFileName.replace(labelFileName.end()-15, labelFileName.end(), "labels.csv");

    string bonnetImgName = frameName;     // eg.'results-fitting/aachen_000000_000019_bonnet.png';
    bonnetImgName.replace(bonnetImgName.end()-15, bonnetImgName.end(), "bonnet.png");

    string pixelLabelFileName = frameName; // eg.'results-fitting/aachen_000000_000019_pixelLabels.csv'; (pixel-level info)
