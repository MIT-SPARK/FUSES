...
/**
 * @file   belief-propagation-LBP.cpp
 * @brief  compute optimization using LBP algorithm
...
int main(int argc, char **argv) {
  ...
  auto start_time = tic();
  fg.computeLBP(labels, iterations, values, times, 40, 1e-5, 0, verbose);
  double end_time = toc(start_time);

  // write to csv
  ofstream outputFile;
  string outputName = argv[1];
  outputName.replace(outputName.end()-3, outputName.end(), "_LBP.csv");
  ...
  cout << "LBP optimal value = " << fg.evaluate(labels)
      << ", time elapsed = " << end_time*1000 << " ms." << endl;
  cout << "Results saved to: " << outputName << endl;
  cout << "LBP code completed successfully!" << endl;
}
