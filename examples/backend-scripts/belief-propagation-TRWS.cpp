...
/**
 * @file   belief-propagation-TRWS.cpp
 * @brief  compute optimization using TRW-S algorithm
...
int main(int argc, char **argv) {
  ...
  outputName.replace(outputName.end()-3, outputName.end(), "_TRWS.csv");
  ...
  cout << "TRW-S optimal value = " << fg.evaluate(labels)
      << ", time elapsed = " << end_time*1000 << " ms." << endl;
  cout << "Results saved to: " << outputName << endl;
  cout << "TRW-S code completed successfully!" << endl;
}
